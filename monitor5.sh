...
# Archivo temporal en el directorio actual
TMP_OLD="./tmp_bundle.css"
cp "$ARCHIVO" "$TMP_OLD"
...
inotifywait -m -e modify "$ARCHIVO" | while read; do
  ...
  if [ -n "$LINE" ]; then
    ...
    diff "$TMP_OLD" "$ARCHIVO" | ccze -A || echo "Sin cambios relevantes"
  else
    echo "⚠️ No se encontró la clave '$CLAVE' en el archivo."
  fi

  # Actualizar copia para próxima comparación
  cp "$ARCHIVO" "$TMP_OLD"
done
